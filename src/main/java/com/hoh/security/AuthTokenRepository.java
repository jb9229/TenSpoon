...
public interface AuthTokenRepository extends JpaRepository<AuthToken, Long>{
    ...
    AuthToken findUserByTokenAndSeries(@Param("token") String token, @Param("series") String series);
    ...
    @Query(nativeQuery = true, value = "delete from auth_token where 1=" +
            "case" +
            "when (last_used_date is null and TIMESTAMPDIFF(MINUTE, created_date, sysdate()) > 2) then 1 " +
            "when (last_used_date <> null and TIMESTAMPDIFF(MINUTE, last_modified_date, sysdate()) > 2) then 2 " +
            "else 0 " + "end ")
    void deleteTimeoutToken();
